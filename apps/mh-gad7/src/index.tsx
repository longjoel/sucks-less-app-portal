...
const STORAGE_PATH = "mh-gad7-results.json";
...
const Preview = (_props: Record<string, never>) => (
  <article>
    <strong>GAD-7 Check-in</strong>
    <p>GAD-7 screening with history, trends, and export.</p>
  </article>
);
...
const MhGad7App = ({ ctx }: { ctx: SlapApplicationContext }) => {
  const [records, setRecords] = useState<ScaleAssessmentRecord[]>([]);
  const [answers, setAnswers] = useState<number[]>(() => new Array(GAD7.questions.length).fill(0));
  ...
  const submitScale = async () => {
    ...
    const nextRecord: ScaleAssessmentRecord = {
      ...
      answers,
      score,
      severity: GAD7.getSeverity(score)
    };

    const next = [nextRecord, ...records].slice(0, 300);
    await persist(next);
    setStatus(`${GAD7.title} saved: ${score}/${GAD7.maxScore} (${nextRecord.severity})`);
    setAnswers(new Array(GAD7.questions.length).fill(0));
  };
  ...
  const exportResults = async () => {
    ...
    anchor.href = href;
    anchor.download = "gad7-results.json";
    anchor.click();
  ...
  return (
    <SlapApplicationShell title="GAD-7 Check-in">
      <SlapApplicationTitle title="GAD-7 Check-in" />
      <SlapInlineText>
      ...
      {latest ? (
        <SlapInlineText>
          Last score: {latest.score}/{GAD7.maxScore} ({latest.severity})
        </SlapInlineText>
      ) : (
      ...
      {history.map((record) => (
        <details key={record.id} className="journal-entry-accordion">
          <summary className="journal-entry-summary">
            <strong>
              {formatDate(record.createdAtIso)} | Score {record.score}/{GAD7.maxScore} ({record.severity})
            </strong>
          </summary>
      ...
      <SlapActionButton title={`Save ${GAD7.title} Result`} onClick={() => void submitScale()} />
    </SlapApplicationShell>
  );
};

export const mhGad7Manifest: SlapApplicationManifest = {
  id: "mh-gad7",
